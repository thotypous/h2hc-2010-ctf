...
abstract class Robot  {
    ...
    protected function initWebRequest() {
        ...
        curl_setopt($this->curl_resource, CURLOPT_COOKIEFILE, $this->cookie_file);
        curl_setopt($this->curl_resource, CURLOPT_COOKIEJAR, $this->cookie_file);
    ...
    protected function breakCaptcha() {
        file_put_contents(($path = tempnam(sys_get_temp_dir(), "CTF_CAPTCHA")), 
            $this->postData(static::CAPTCHA_IMG));
        $result = $this->captchaTreatment($path);
        unlink($path);
        return $result;
    }
